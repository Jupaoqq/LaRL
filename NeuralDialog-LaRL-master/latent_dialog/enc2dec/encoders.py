...
class EncoderRNN(BaseRNN):
    ...
    def forward(self, input_var, init_state=None, input_lengths=None, movies=None):
        # add goals
        if movies is not None:
            batch_size, max_ctx_len, ctx_nhid = input_var.size()
            movies = movies.view(movies.size(0), 1, movies.size(1))
            movies_rep = movies.repeat(1, max_ctx_len, 1).view(batch_size, max_ctx_len, -1) # (batch_size, max_ctx_len, goal_nhid)
            input_var = th.cat([input_var, movies_rep], dim=2)
...
class RnnUttEncoder(nn.Module):
    def __init__(self, vocab_size, embedding_dim, feat_size, movie_nhid, rnn_cell,
        ...
        self.rnn = EncoderRNN(input_dropout_p=input_dropout_p,
                              rnn_cell=rnn_cell, 
                              input_size=embedding_dim+feat_size+movie_nhid, 
    ...
    def forward(self, utterances, feats=None, init_state=None, movies=None):
        ...
        # add goals
        if movies is not None:
            movies = movies.view(movies.size(0), 1, 1, movies.size(1))
            movies_rep = movies.repeat(1, max_ctx_len, max_utt_len, 1).view(batch_size*max_ctx_len, max_utt_len, -1) # (batch_size*max_ctx_len, max_utt_len, movie_nhid)
            word_embeddings = th.cat([word_embeddings, movies_rep], dim=2)
...
class MlpGoalEncoder(nn.Module):
    def __init__(self, movie_vocab_size, k, nembed, nhid, init_range):
        super(MlpGoalEncoder, self).__init__()

        # create separate embedding for counts and values
        self.cnt_enc = nn.Embedding(movie_vocab_size, nembed)
        self.val_enc = nn.Embedding(movie_vocab_size, nembed)
    ...
    def forward(self, movie):
        ...
        print("movie")
        print(movie)
        movie = movie.transpose(0, 1).contiguous() # (goal_len, batch_size)
        print(movie)
        idx = np.arange(movie.size(0) // 4)
        ...
        cnt = movie.index_select(0, cnt_idx) # (3, batch_size)
        val = movie.index_select(0, val_idx) # (3, batch_size)
        ...
        h = h.transpose(0, 1).contiguous().view(movie.size(1), -1) # (batch_size, 3*nembed)
        movie_h = self.encoder(h) # (batch_size, nhid)

        return movie_h


class TaskMlpGoalEncoder(nn.Module):
    def __init__(self, movie_vocab_sizes, nhid, init_range):
        super(TaskMlpGoalEncoder, self).__init__()
        
        self.encoder = nn.ModuleList()
        for v_size in movie_vocab_sizes:
    ...
    def forward(self, movies_list):
        # goals_list: list of tensor, 7*(batch_size, goal_len), goal_len varies among differnet domains
        outs = [encoder.forward(movie) for movie, encoder in zip(movies_list, self.encoder)] # 7*(batch_size, goal_nhid)
        outs = th.sum(th.stack(outs), dim=0) # (batch_size, movie_nhid)
        return outs
