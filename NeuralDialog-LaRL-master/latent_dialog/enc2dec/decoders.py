...
class DecoderRNN(BaseRNN):
    ...
    def forward(self, batch_size, dec_inputs, dec_init_state, attn_context, mode, gen_type, beam_size, movie_hid=None):
        # dec_inputs: (batch_size, response_size-1)
        # attn_context: (batch_size, max_ctx_len, ctx_cell_size)
        # movie_hid: (batch_size, goal_nhid)
        ...
        if mode == TEACH_FORCE:
            prob_outputs, decoder_hidden_state, attn = self.forward_step(input_var=decoder_input, hidden_state=decoder_hidden_state, encoder_outputs=attn_context, movie_hid=movie_hid)
        else:
            ...
            for step in range(self.max_dec_len):
                ...
                decoder_output, decoder_hidden_state, step_attn = self.forward_step(decoder_input, decoder_hidden_state, attn_context, movie_hid=movie_hid)
    ...
    def forward_step(self, input_var, hidden_state, encoder_outputs, movie_hid):
        ...
        # add goals
        if movie_hid is not None:
            movie_hid = movie_hid.view(movie_hid.size(0), 1, movie_hid.size(1)) # (batch_size, 1, goal_nhid)
            goal_rep = movie_hid.repeat(1, output_seq_len, 1) # (batch_size, output_seq_len, goal_nhid)
    ...
    # special for rl
    def _step(self, input_var, hidden_state, encoder_outputs, movie_hid):
        # input_var: (1, 1)
        # hidden_state: tuple: (h, c)
        # encoder_outputs: (1, max_dlg_len, dlg_cell_size)
        # movie_hid: (1, goal_nhid)
        batch_size, output_seq_len = input_var.size()
        embedded = self.embedding(input_var) # (1, 1, embedding_dim)

        if movie_hid is not None:
            movie_hid = movie_hid.view(movie_hid.size(0), 1, movie_hid.size(1)) # (1, 1, goal_nhid)
            goal_rep = movie_hid.repeat(1, output_seq_len, 1) # (1, 1, goal_nhid)
    ...
    # special for rl
    def write(self, input_var, hidden_state, encoder_outputs, max_words, vocab, stop_tokens, movie_hid=None, mask=True,
        ...
        for i in range(max_words):
            decoder_output, decoder_hidden_state = self._step(decoder_input, decoder_hidden_state, encoder_outputs, movie_hid)
    ...
    # For MultiWoz RL
    def forward_rl(self, batch_size, dec_init_state, attn_context, vocab, max_words, movie_hid=None, mask=True, temp=0.1):
        # prepare the BOS inputs
        with th.no_grad():
            bos_var = Variable(th.LongTensor([self.sys_id]))
        bos_var = cast_type(bos_var, LONG, self.use_gpu)
