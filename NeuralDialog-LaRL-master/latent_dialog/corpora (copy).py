...
class DealCorpus(object):
    ...
    def _process_dialogue(self, data):
        ...
        for raw_dlg in data:
            ...
            #     exit(-1)
            user_response = raw_words[raw_words.index('<user>') + 1: raw_words.index('</user>')]
            movieID = []
            suggest = []
            seen = []
            like = []
            ...
            # print(json.dumps(Pack(dlg=cur_dlg, goal=cur_goal, out=cur_out, response = dicts), indent = 4))

            new_dlg.append(Pack(dlg=cur_dlg, goal=cur_goal, out=cur_out, response = dicts))
    ...
    def _to_id_corpus(self, name, data):
        ...
        for dlg in data:
            ...
            # print(json.dumps(Pack(dlg=id_dlg, goal=id_goal, out=id_out, response = dlg.response), indent = 4))

            results.append(Pack(dlg=id_dlg, goal=id_goal, out=id_out, response = dlg.response))
        return results
